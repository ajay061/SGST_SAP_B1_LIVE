...
{
    public partial class WIPFQAReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["usertype"].ToString() != "ADMIN")
            {
                string _strRights = CommonHelper.GetRights("FQA REPORT", (DataTable)Session["USER_RIGHTS"]);
        ...
        private void BindPart_No()
        {
            try
            {
                BL_WIPFQAReport blobj;
                blobj = new BL_WIPFQAReport();
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dtTrackingData = blobj.GetReport(sPartCode, txtWorkOrderNo.Text.Trim(), txtFromDate.Text, txtToDate.Text, drpReportType.Text);
                ...
                else
                {
                    ...
                    CommonHelper.mBcilLogger.LogMessage(BcilLib.EventNotice.EventTypes.evtMessage,
                         System.Reflection.MethodBase.GetCurrentMethod().Name, "FQA Report No of records :" + dtTrackingData.Rows.Count);
                    ReportViewer1.ProcessingMode = ProcessingMode.Local;
                    ReportDataSource datasource = new ReportDataSource();
                    if (drpReportType.Text == "Summary")
                    {
                        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/ReportViewer/WIPFQASummaryReport.rdlc");
                    }
                    else
                    {
                        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/ReportViewer/WIPFQAReport.rdlc");
                    }
