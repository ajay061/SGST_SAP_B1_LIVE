...
{
    public partial class PalletLabelNew : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["usertype"].ToString() != "ADMIN")
            {
                string _strRights = CommonHelper.GetRights("PACKING LABEL REPORT", (DataTable)Session["USER_RIGHTS"]);
        ...
        private void BindPackingListNo()
        {
            try
            {
                BL_Pallet_Label_Report blobj;
                blobj = new BL_Pallet_Label_Report();
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                BL_Pallet_Label_Report blobj;
                blobj = new BL_Pallet_Label_Report();
                dtGetReport = blobj.GetReport(sPackingListNo);
                if (dtGetReport.Rows.Count > 0)
                {
                    ReportViewer1.ProcessingMode = ProcessingMode.Local;
                    ReportDataSource datasource = new ReportDataSource();
                    CommonHelper.HideMessage(msginfo, msgsuccess, msgwarning, msgerror);
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/ReportViewer/FG Reports/PALLETRDLC.rdlc");
